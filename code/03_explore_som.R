...
ggsave('./results/plots/01_nodes_4_vars_boxplot.png', g1, 'png', 
...
dev.copy(png, './results/plots/01_nodes_4_vars_radar.png', 
         width = 800, height = 800)
dev.off()

#Months per group
group_months <- group_months[unique(to_plot_groups[, .(P_group, order_id)]), on = 'P_group']
...
ggsave('./results/plots/01_nodes_month_main_4_vars.pdf', g2, 'pdf', 
       width = 30, height = 20, units = 'cm')

#Runoff groups vs SOM nodes
to_plot <- ros_subset[, .N, .(P_group, Q_group)]
#to_plot <- group_order_id[to_plot, on = 'P_group']
...
ggsave('./results/plots/01_nodes_Qgroupmain_4_vars.pdf', g3, 'pdf', 
       width = 30, height = 20, units = 'cm')
...
to_plot <- to_plot_groups[, .N, .(P_group, cl_group_label)]
...
to_plot <- dummy[, .N, .(P_group, catchment)]
to_plot <- to_plot[, perc := N / sum(N), P_group]
...
to_plot <- dummy[, .N, .(P_group, mountain)]
to_plot <- to_plot[, perc := N / sum(N), P_group]
to_plot <- group_order_id[to_plot, on = 'P_group']
