...
# A 3 x 3 SOM is chosen with no further classification (9 groups)
...
ros_subset[, quantile(EventPrec)]

ros_subset <- ros_subset[EventPrec >= 22.2] #Above 75%

#Homogenization
ros_for_som <- as.matrix(apply(ros_subset[, -1], 2, scale)) # ID column should not be used in cluster analysis
ros_for_som <- ros_for_som[, 1:(ncol(ros_for_som) - 1)]

cor(ros_for_som) # To check for variables that have cor > 0.7
...
save(ros_som, ros_subset, my_palette, n_var, n_nodes, ros, file = './data/som_3x3_main_vars_snow_area')
...
load('./data/som_3x3_main_vars_snow_area')

groups <- 9 # number of groups with similar properties | each node is a group
