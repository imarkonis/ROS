...
ros_subset <- ros[, .(ID,
                      ...
                      EventPrec,
                      Rain_int = EventPrec/RainDuration, #precipitation intensity
                      Q_int = TotalQ/Event_length,    #runoff intensity
...
save(ros_som, ros_subset, ros_cat, my_palette, n_var, n_nodes, ros, file = './data/som_4x4_main_6_new_vars_rev.rdata')
save(ros_som, ros_subset, ros_cat, my_palette, n_var, n_nodes, ros, file = 'som_4x4_main_6_new_vars_rev.rdata') #Alternative path
write.csv(ros, file = 'ros_som_6_new.csv')
