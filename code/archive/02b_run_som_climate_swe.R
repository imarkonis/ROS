...
ros_subset <- ros[, .(ID,
                      SWE.mean,
                      AveTLo,          #the air temperature
                      EventPrec,       #precipitation amount  
                      RainDuration,    #length of the RE in hours
...
save(ros_som, ros_subset, my_palette, n_var, n_nodes, ros, file = './data/som_4x4_climate_swe')

#Hierarchical clustering for further classification
load('./data/som_4x4_climate')
...
ros_subset$cl_group <- as.factor(as.numeric(ros_group))
...
ros_subset[, n_ros_group := .N, by = cl_group] #number of ros events per hclust group

to_plot_groups <- data.table(melt(ros_subset[, 2:(n_var + 1)], 
                                  id.vars = c('cl_group'))) #tidy up
dummy <- ros_subset[, 2:(n_var + 1)]
group_means <- dummy[, lapply(.SD, mean, na.rm = T), by = cl_group] 

group_means <- group_means[order(EventPrec), ]
group_means$order_id <- factor(1:nrow(group_means))
to_plot <- data.table(melt(group_means, id.vars = c('order_id', 'cl_group'))) #tidy up
group_means <- group_means[order(cl_group), ] 
dummy <- unique(to_plot[, .(cl_group, order_id)])
to_plot_groups <- dummy[to_plot_groups, on = c('cl_group')]

ggplot(to_plot_groups, aes(x = order_id, y = value, fill = cl_group)) +  #ploting boxplots ordered by EventMelt means
  geom_boxplot() +
...
ros_months <- ros_subset[ros[, .(ID, 
                         ...
                         on = 'ID']
group_months <- ros_months[, .N, .(cl_group, month)]
group_months <- group_months[complete.cases(group_months)]
most_events_month <- group_months[group_months[, .I[N == max(N)], by = cl_group]$V1]
most_events_month <- most_events_month[order(cl_group)]

# Group characteristics with radar chart
group_var_range <- apply(group_means, 2, function(x) range(as.numeric(x)))
n_ros_group <- unique(ros_subset[, .(cl_group, n_ros_group)])
n_ros_group <- n_ros_group[order(cl_group), ]

ros_subset[, cl_group_label := cl_group]

ros_subset[, cl_group_label := factor(cl_group_label, 
                                       labels = c("LP MT", "MP MT", "MP LT", 
                                                  "LP HT", "HP LT", "MP HT"))]
op <- par(mar = c(1, 1, 1, 1), 
          mfrow = c(3, 2))

for(group_id in 1:groups){
  group_for_radar <- rbind(group_var_range[2:1, ], group_means[cl_group == group_id, ])[, 2:n_var]
...
dev.copy(pdf, './results/plots/00_radar_climate_groups.pdf')
dev.off()

#Groups per month
ggplot(group_months, aes(x = as.factor(cl_group), y = N, fill = as.factor(cl_group))) +
...
saveRDS(ros_subset, './data/climate_groups.rds')

#Runoff groups
q_groups <- ros[, .(ID, Q_group)]
ros_subset <- ros_subset[q_groups, on = 'ID']
ros_groups <- ros_subset[, .(cl_group, Q_group)]

to_plot <- ros_groups[, .N, .(cl_group, Q_group)]

ggplot(to_plot, aes(x = as.factor(cl_group), y = N, fill = as.factor(cl_group))) +
  geom_bar(stat = 'identity') +
  facet_wrap(~Q_group) +
  coord_polar() +
  scale_fill_manual(values = my_palette(12)) +
...
colnames(ros_subset)[7] <- 'cl_node'

saveRDS(ros_subset[, .(ID, cl_group, cl_node)], "./data/climate_SOM.rds")
